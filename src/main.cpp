...
int nCoinbaseMaturity = 1;//mining need 30 confirm
...
const string strMessageMagic = "BitcoinBuck Signed Message:\n";
...
int64 GetProofOfStakeReward(int64 nCoinAge, unsigned int nBits, unsigned int nTime)
{
    ...
    // BitcoinBuck: reward for coin-year is cut in half every 64x multiply of PoS difficulty
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal bitcoinbuck miner
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in BitcoinBuck
    bool fStrictPayToScriptHash = true; // Always active in BitcoinBuck
    ...
    // ppcoin: fees are not collected by miners as in bitcoinbuck
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "BitcoinBuck", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "Cointelegraph: August 1, 2021 - PayPal reportedly assembling crypto team in Ireland as Bitcoin adoption grows.";
        CTransaction txNew;
        ...
        txNew.strTxComment = "text:BitcoinBuck genesis block";
        ...
        assert(block.hashMerkleRoot == uint256("0x9382ba72c2846fa0d4433ab5e0501facbd83b40ba1a5eea8cf0e1df5abea18d6"));
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (hash > hashTarget && pblock->IsProofOfWork())
        return error("BitcoinBuckMiner : proof-of-work not meeting target");

    //// debug print
    printf("BitcoinBuckMiner:\n");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BitcoinBuckMiner : generated block is stale");
        ...
        if (!ProcessBlock(NULL, pblock))
            return error("BitcoinBuckMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ThreadBitcoinBuckMiner(void* parg);

static bool fGenerateBitcoinBucks = false;
static bool fLimitProcessors = false;
static int nLimitProcessors = -1;

void BitcoinBuckMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    RenameThread("bitcoinbuck-miner");

    // Each thread has its own key and counter
    CReserveKey reservekey(pwallet);
    unsigned int nExtraNonce = 0;

    while (fGenerateBitcoinBucks || fProofOfStake)
    {
        if (fShutdown)
            return;
        while (vNodes.empty() || IsInitialBlockDownload())
        {
            ...
            if (fShutdown)
                return;
            if ((!fGenerateBitcoinBucks) && !fProofOfStake)
                return;
        }
        ...
        printf("Running BitcoinBuckMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinBuckMiner(void* parg)
{
    ...
    try
    {
        ...
        BitcoinBuckMiner(pwallet, false);
        vnThreadsRunning[THREAD_MINER]--;
    }
    catch (std::exception& e) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(&e, "ThreadBitcoinBuckMiner()");
    } catch (...) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(NULL, "ThreadBitcoinBuckMiner()");
    }
    nHPSTimerStart = 0;
    if (vnThreadsRunning[THREAD_MINER] == 0)
        dHashesPerSec = 0;
    printf("ThreadBitcoinBuckMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}


void GenerateBitcoinBucks(bool fGenerate, CWallet* pwallet)
{
    fGenerateBitcoinBucks = fGenerate;
    nLimitProcessors = GetArg("-genproclimit", -1);
    if (nLimitProcessors == 0)
        fGenerateBitcoinBucks = false;
    ...
    if (fGenerate)
    {
        ...
        printf("Starting %d BitcoinBuckMiner threads\n", nAddThreads);
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!NewThread(ThreadBitcoinBuckMiner, pwallet))
                printf("Error: NewThread(ThreadBitcoinBuckMiner) failed\n");
            Sleep(10);
        }
    }
}
